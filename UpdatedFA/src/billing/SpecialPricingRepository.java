...
public class SpecialPricingRepository {
    ...
    private String displayRole(String roleCode) {
        if ("SD".equals(roleCode)) {
            return "Student";
        }

        if ("LC".equals(roleCode)) {
            return "Lecturer";
        }

        if ("AD".equals(roleCode)) {
            return "Admin";
        }
        ...
        try (PreparedStatement statement = conn.prepareStatement(sql);
            ...
            while (result.next()) {
                records.add(new SpecialPricingRecord(
                        ...
                        displayRole(result.getString("role_name")),
    ...
    public void add(int roleId, String name, String type, String operator, BigDecimal value)
        ...
        try {
            ...
            String roleSql =
                    "INSERT INTO roleadjustments(role, pricing_id) VALUES (?, ?)";
    ...
    public void update(int pricingId, int roleId, String name, String type,
        ...
        try {
            ...
            String pricingSql =
                    "UPDATE rolepricing " +
                            "SET pricing_name = ?, pricing_type = ?, operator = ?, price_adjustment = ? " +
                            "WHERE pricing_id = ?";
            ...
            String roleSql =
                    "UPDATE roleadjustments SET role = ? WHERE pricing_id = ?";
    ...
    public void delete(int pricingId) throws SQLException {
        ...
        try {
            ...
            String deleteRoleAdjustmentSql =
                    "DELETE FROM roleadjustments WHERE pricing_id = ?";

            try (PreparedStatement statement = conn.prepareStatement(deleteRoleAdjustmentSql)) {
                statement.setInt(1, pricingId);
                statement.executeUpdate();
            }

            String deletePricingSql =
                    "DELETE FROM rolepricing WHERE pricing_id = ?";

            try (PreparedStatement statement = conn.prepareStatement(deletePricingSql)) {
                statement.setInt(1, pricingId);
                statement.executeUpdate();
            }
