...
public class Login extends JPanel {
    ...
    private JPanel loginGuiInit() {
        JPanel loginGUI = new JPanel(new GridBagLayout());
        ...
        for (int i = 0; i < 4; i++) {
            ...
            roleButtons[i].setFont(new Font("Dialog", Font.PLAIN, 36));
            roleButtons[i].setAlignmentX(Component.CENTER_ALIGNMENT);
            roleButtons[i].setMaximumSize(new Dimension(420, 65));
            roleButtons[i].setPreferredSize(new Dimension(420, 65));

            rolePanel.add(roleButtons[i]);

            if (i < 3) {
                rolePanel.add(Box.createVerticalStrut(25));
            }
        }

        loginGUI.add(rolePanel);
    ...
    private JPanel enterCreds() {
        ...
        wrapper.setBorder(BorderFactory.createEmptyBorder(30, 200, 50, 200));

        JPanel enterCreds = new JPanel(new GridLayout(4, 1, 0, 30));
        enterCreds.setPreferredSize(new Dimension(520, 420));
        ...
        usernameBoxLabel.setFont(new Font("Dialog", Font.PLAIN, 28)); // make font size bigger
        JTextField enterUser = new JTextField(); // box to enter username
        enterUser.setFont(new Font("Dialog", Font.PLAIN, 28)); // make font size bigger
        ...
        passwordBoxLabel.setFont(new Font("Dialog", Font.PLAIN, 28)); // make font size bigger
        enterPass.setFont(new Font("Dialog", Font.PLAIN, 28)); // make font size bigger
        ...
        confLogin.setFont(new Font("Dialog", Font.PLAIN, 28)); // make button font size bigger
        cancLogin.addActionListener(new returnListener()); // attach listener to button
        cancLogin.setFont(new Font("Dialog", Font.PLAIN, 28)); // make button font size bigger
    ...
    private void loginToDash(String username) {
        try {
            if (role.equals("AD")) {
                // admin account is stored in administrator table, not sysUser
                String adminString = "SELECT admin_id FROM administrator WHERE admin_username = (?)";
                PreparedStatement adminStatement = conn.prepareStatement(adminString);
                adminStatement.setString(1, username);
                ResultSet result = adminStatement.executeQuery();

                if (result.next()) {
                    int adminId = result.getInt("admin_id");
                    currUser = new Session("AD", 0, adminId, username, username);
                }
                ...
                if (result.next()) {
                    ...
                    currUser = new Session(role, roleInt, userId, username, userName);
                }
