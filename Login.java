...
public class Login extends JFrame {
    ...
    public Login() {
        ...
        // NOTE: super(...) must be the very first statement in a constructor,
        // so the database connection is fetched after this instead of before it.

        this.conn = DBConnection.getConnection(); // shared connection, see DBConnection.java
            ...
            public void actionPerformed(ActionEvent e) {
                ...
                switch (roleInt) {
                    ...
                    case 1:
                        ...
                        // (was incorrectly card3 'Login as Doctor')
                    ...
                    case 2:
                        ...
                        // (was incorrectly card4 'Login as Receptionist')
        ...
        class loginListener implements ActionListener { // listener for login button
            public void actionPerformed(ActionEvent e) {
                ...
                // passed all checks, proceed to login
                    ...
                    completeLogin(usernameInput, passwordInput); // loads the user's details into Session

                    JOptionPane.showMessageDialog(null,
                            "Login Successful, Welcome " + Session.getName() + ".",
                            "Login Success", JOptionPane.INFORMATION_MESSAGE);

                    openDashboard(); // opens the right dashboard for this role and closes the Login window
                }
            }
        }
    ...
    // checks whether username+password+role match a row in the database
    ...
    // fetches this user's id and full name and stores them in Session so the
    // dashboards (and later, appointment booking etc.) know who is logged in
    private void completeLogin(String username, String password) {
        String detailString = "SELECT user_id, user_name FROM hmsUser WHERE user_role = (?) AND user_username = (?) AND user_password = (?)";
        try {
            PreparedStatement detailStatement = conn.prepareStatement(detailString);
            detailStatement.setString(1, role);
            detailStatement.setString(2, username);
            detailStatement.setString(3, password);
            ResultSet result = detailStatement.executeQuery();

            if (result.next()) {
                int userId = result.getInt("user_id");
                String userName = result.getString("user_name");
                Session.setCurrentUser(role, userId, username, userName);
            }
    ...
    // opens the correct dashboard based on the role chosen at login, then
    ...
    private void openDashboard() {
        ...
        dispose(); // close the Login window now that we're heading to the dashboard
    }
