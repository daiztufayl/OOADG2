...
public class Login extends JPanel {
    ...
    private void loginToDash(String username) {
        try {
            if (role.equals("AD")) {
                ...
                JOptionPane.showMessageDialog(null, "User dashboard not created yet.");
            }
        } catch (SQLException e) {
            e.printStackTrace();
        }
    ...
    // standalone entry point: lets you run/test just the Login screen directly
    public static void main(String[] args) {
        SwingUtilities.invokeLater(new Runnable() {
            @Override
            public void run() {
                new RoleLookup(); // load roles before opening login directly

                JFrame testFrame = new JFrame("Login Test");
                testFrame.setDefaultCloseOperation(JFrame.EXIT_ON_CLOSE);
                testFrame.setSize(900, 900);
                testFrame.add(new Login());
